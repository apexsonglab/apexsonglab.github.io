...
export default defineConfig({
  ...
  // Native Fonts API: self-hosts + subsets + preloads the two typefaces and
  // generates metric-adjusted fallbacks. Injected via <Font /> in Layout.astro
  // and consumed through the CSS variables repointed in CustomStyles.astro.
  //
  // Ovo (serif) sets headings, Almarai (sans) sets body and UI text.
  fonts: [
    {
      provider: fontProviders.fontsource(),
      name: 'Almarai',
      cssVariable: '--font-almarai',
      weights: [400, 700],
      styles: ['normal'],
      subsets: ['latin'],
